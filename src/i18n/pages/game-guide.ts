...
export const gameGuideContent: Record<Locale, GameGuideContent> = {
  "zh-Hans": {
    ...
    status: {
      ...
      body:
        "现有原型已经具备统一运行入口、无人运行、观察者路径、存档回放和因果记录基础，但角色认知、随机流、公式出处与研究导出仍需补全。网站中的历史模拟模式内容描述目标合同，不代表这些能力已经全部可用。",
    },
    articleDetail: {
      backLabel: "返回游戏指南",
      contentsLabel: "本页内容",
      ...
      },
      {
        ...
        sections: [
          ...
          },
          {
            ...
            layout: "grid",
            items: [
              { label: "已有基础", text: "统一运行入口、无人运行、观察者路径、存档回放、世界哈希与部分因果记录。" },
  ...
  en: {
    ...
    status: {
      ...
      body:
        "The prototype already has a shared run entry, unattended runs, an observer path, save/replay, and causal-record foundations. Actor knowledge, random streams, formula provenance, and research exports still need substantial work. The Historical Simulation Mode pages describe the target contract, not a claim that every capability is available today.",
    },
    articleDetail: {
      ...
      },
      {
        ...
        sections: [
          ...
          },
          {
            ...
            layout: "grid",
            items: [
              { label: "Existing base", text: "Shared run entry, unattended execution, observer path, save/replay, world hashes, and partial causal records." },
