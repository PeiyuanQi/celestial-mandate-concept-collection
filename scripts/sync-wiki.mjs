...
const categories = [
  ...
  },
  {
    ...
    sourceFiles: [
      ...
      {
        path: "open/natural-earth/README.md",
        slug: "natural-earth-reference-collection",
      },
      {
        path: "open/geonames/README.md",
        slug: "geonames-reference-collection",
      },
      {
        path: "paradox-grand-strategy/phase1-demo-cross-reference.md",
...
function collectExplicitMarkdownSources(category, sourceDir) {
  return category.sourceFiles.map((sourceFile) => ({
    name: sourceFile.path,
    slug: sourceFile.slug,
    absolutePath: resolve(sourceDir, sourceFile.path),
    fallbackTitleFilename: sourceFile.path.split(/[\\/]/).pop(),
    sourcePathAliases: [],
  }));
}
...
function collectMarkdownSources(category, sourceDir) {
  if (category.sourceFiles) {
    return collectExplicitMarkdownSources(category, sourceDir);
  }
...
function writeGeneratedContent(entries) {
  ...
  const publicCategories = categories.map(
    ({ sourceFiles: _sourceFiles, ...category }) => category,
  );
  ...
  writeFileSync(
    ...
    `// Generated by npm run sync-wiki. Do not edit by hand.\n` +
      ...
      `export const wikiCategoryGroups = ${JSON.stringify(publicCategories, null, 2)} as const;\n\n` +
