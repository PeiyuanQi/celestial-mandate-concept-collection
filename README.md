...
- Public wiki entries are generated from markdown under `../celestial-mandate/references/`. Update that source first, then run `npm run sync-wiki` to refresh `src/i18n/wiki/generated.ts` and `public/wiki-content/`. People entries read canonical biographies from `people/<slug>/profile.md` and publish them as stable `people/<slug>.json` wiki entries. They can also display same-slug portrait art from `people/<slug>/portrait.png` with optional `portrait.md` provenance; generated portrait art is a visual concept asset, not historical evidence.
