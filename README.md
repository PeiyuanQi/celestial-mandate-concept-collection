...
- The preferred personal-domain URL is `https://configured external website target`; build for that path with `npm run build-website`, then publish the resulting `dist/` folder through the `external website repo` site.
...
- Verify: run `npm run build` for website changes, or `npm run build-website` when preparing the personal-domain deploy output. For documentation-only changes, run `git diff --check`.
