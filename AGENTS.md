...
- The public wiki is generated from markdown under `../celestial-mandate/references/`. When asked to update wiki entries, inspect that source first, then run `npm run sync-wiki`; the current public categories are people, events, places, and chronicle/timeline. People pages use `people/<slug>/profile.md` as the canonical biography source while publishing stable `people/<slug>.json` wiki entries. They may also sync a same-slug `people/<slug>/portrait.png` plus optional `portrait.md` provenance note into the public wiki reader. Do not sync raw archives, scans, ebooks, binaries, or unrestricted reference folders into the website, and do not treat generated portrait art or portrait provenance notes as historical evidence.
